...
Este guia descreve **cada passo**, na ordem certa, para subir o pacote
ao GitHub com pkgdown e em seguida submetê-lo ao CRAN.
...
`https://raydonal.github.io/prLogistic/` após o primeiro push (o GitHub
Actions cuida disso).
...
Confirme que `NAMESPACE` foi atualizado e que os arquivos `.Rd` foram
criados em `man/`.
...
Crie este arquivo na raiz do pacote (não é enviado ao CRAN, mas é boa
prática manter):
...
**Campos obrigatórios no formulário:** - Name: Raydonal Ospina - Email:
<raydonal@de.ufpe.br> - Upload: `prLogistic_2.0.0.tar.gz`
...
É muito comum o CRAN solicitar ajustes. As solicitações mais frequentes
são:
...
### 4.2 “Please add to all exported functions”
...
Teste todas as URLs manualmente. O CRAN verifica literalmente cada
`\url{}` e `\doi{}` nos arquivos `.Rd`.
