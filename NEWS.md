...
* The `cluster` and `pattern` arguments of the old `prLogisticDelta()` are
...
* Return value is now a `prLogistic` S3 object rather than a plain matrix,
  with `print`, `summary`, `coef`, `confint`, and `plot` methods.

## New features

* **Continuous covariates**: reference value defaults to the sample median
  (configurable via `ref_continuous = "mean"`). The old implementation only
...
* **GEE support** (`prLogisticGEE()`): prevalence ratios for longitudinal /
  repeated-measures data fitted with `geepack::geeglm()`. Robust (sandwich)
  variance is used automatically.
* **Complex survey support** (`prLogisticSurvey()`): prevalence ratios for
...
* Fixed incorrect variance calculation in `pr_marginal()` for models with
...
* Replaced `lmer` class with `glmerMod` for compatibility with lme4 ≥ 1.0-4.

# prLogistic 1.1 (2011-10-25)

* Added `NAMESPACE` and `ChangeLog` files.
* Replaced `mean` with `colMeans()` in `pr.marginal()`.
* Modified `vcov()` structure for S4 compatibility.
